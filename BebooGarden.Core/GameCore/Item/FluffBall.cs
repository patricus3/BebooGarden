...
public class FluffBall : Item
{
  ...
  private void GoHome(Map from)
  {
    ...
    if (GameHost.Current.Map == from || GameHost.Current.Map == Map.Fluff)
      Voice.Current.Say(BebooText.fluffball_goeshome);
  }
  ...
  public static void FollowFriend(Beboo beboo, Map? from, Map to)
  {
        ...
        .Where(ball => ball.FriendName == beboo.Name).ToList())
    {
      ...
      Voice.Current.Say(String.Format(BebooText.fluffball_follows, beboo.Name));
    }
  }
  ...
  public override void Action()
  {
    ...
    GameHost.Current.SoundSystem.PlayFluffBallSound(GameHost.Current.SoundSystem.FluffBallHugSounds, this);
    Voice.Current.Say(BebooText.fluffball_hugyou);
  }
  ...
  public override void BebooAction(Beboo beboo)
  {
    ...
    beboo.Happiness++;
    GameHost.Current.SoundSystem.PlayFluffBallSound(GameHost.Current.SoundSystem.FluffBallHugSounds, this);
    Voice.Current.Say(String.Format(BebooText.fluffball_hugbeboo, beboo.Name));
    MaybeAttachTo(beboo);
  }
  ...
  private void MaybeAttachTo(Beboo beboo)
  {
    ...
    if (GameHost.Current.Random.Next(6) != 0) return;
    ...
    GameHost.Current.SoundSystem.PlayFluffBallSound(GameHost.Current.SoundSystem.FluffBallHugSounds, this);
    Voice.Current.Say(String.Format(BebooText.fluffball_attached, beboo.Name));
  }
  ...
  public override void Update()
  {
    ...
    if (MurmurBehaviour.ItsTime())
    {
      ...
      GameHost.Current.SoundSystem.PlayFluffBallSound(
          GameHost.Current.SoundSystem.FluffBallMurmurSounds, this,
  ...
  {
    Beboo? friend = Friend;
    if (friend != null && (GameHost.Current.Map?.Beboos.Contains(friend) ?? false)
        && !(GameHost.Current.Map?.IsInWater(friend.Position) ?? false))
      return friend.Position;
    return GameHost.Current.Map?.GenerateRandomUnoccupedPosition(excludeWater: true);
  }
  ...
  private void NoticeWhetherAlone()
  {
    ...
    GameHost.Current.SoundSystem.PlayFluffBallSound(
        alone ? GameHost.Current.SoundSystem.FluffBallMurmurSounds
              : GameHost.Current.SoundSystem.FluffBallHugSounds,
  ...
  private void Drift()
  {
    ...
    if (GameHost.Current.Map?.IsInWater(next) ?? false) return;
    Position = next;
  }
  ...
  {
    if (Position == null || !ReadyToHug || GameHost.Current.Map == null) return;
    FluffBall? other = GameHost.Current.Map.Items.OfType<FluffBall>().FirstOrDefault(ball =>
