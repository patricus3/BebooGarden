...
public class Bubble : Item
{
  ...
  {
    GameHost.Current.SoundSystem.PlaySoundAtPosition(GameHost.Current.SoundSystem.BubbleSounds, Position.Value);
    if (GameHost.Current.Random.Next(5) == 1)
    {
      GameHost.Current.SoundSystem.PlaySoundAtPosition(GameHost.Current.SoundSystem.BubblePopSound, Position.Value);
      GameHost.Current.Map?.Items.Remove(this);
      MoveBehaviour.Stop();
      SlowDriftBehaviour.Stop();
    }
    else Direction = Util.DIRECTIONS[GameHost.Current.Random.Next(Util.DIRECTIONS.Length)];
  }
  ...
  public override void Update()
  {
    ...
    if (MoveBehaviour.ItsTime())
    {
      ...
      {
        List<Item> bubbles = GameHost.Current.Map.Items.FindAll(x => x is Bubble);
    ...
    if (SlowDriftBehaviour.ItsTime()) {
      if (Direction != null || Position == null) return;
      Direction = Util.DIRECTIONS[GameHost.Current.Random.Next(Util.DIRECTIONS.Length)];
      GameHost.Current.SoundSystem.PlaySoundAtPosition(GameHost.Current.SoundSystem.BubbleUpSound, Position.Value);
