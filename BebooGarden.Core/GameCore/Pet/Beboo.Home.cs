...
public partial class Beboo
{
  ...
  {
    List<Beboo> around = GameHost.Current.Map?.GetBeboosArround(Position) ?? [];
  ...
  private bool NextToANest() => GameHost.Current.Map?.Items.OfType<Nest>().Any(nest =>
  ...
  private void AnnounceSnuggle(Beboo friend)
  {
    ...
    Voice.Current.Say(String.Format(BebooText.beboo_snuggle, Name, friend.Name));
    GameHost.Current.SoundSystem.PlayBebooSound(GameHost.Current.SoundSystem.BebooDelightSounds, this, true, 0.3f);
  }

  public void ForgetSnuggling() => _snuggledWith = null;
  ...
  public void StartErrand()
  {
    ...
    Vector3? spot = GameHost.Current.Map?.GenerateRandomUnoccupedPosition(true);
    if (spot == null) return;
  ...
  public void UpdateErrand()
  {
    ...
    if (_errand == Errand.Fetching)
    {
      ...
      _presentFruit = GameHost.Current.Random.Next(5) == 0
          ? null
          : GameHost.Current.Random.Next(4) == 0 ? FruitSpecies.Energetic : FruitSpecies.Normal;
    ...
    if (!Util.IsInSquare(Position, GameHost.Current.PlayerPosition, 1))
    {
  ...
  private void Deliver()
  {
    ...
    {
      Voice.Current.Say(String.Format(BebooText.beboo_presentticket, Name));
      GameHost.Current.GainTicket(1);
      return;
    }
    FruitSpecies fruit = _presentFruit.Value;
    var basket = GameHost.Current.Save.FruitsBasket;
    if (basket != null) basket[fruit] = basket.GetValueOrDefault(fruit) + 1;
    GameHost.Current.SoundSystem.DropFruitSound(fruit);
    Voice.Current.Say(String.Format(BebooText.beboo_present, Name, Util.Localized(fruit.ToString())));
  }

  public void AbandonErrand() => _errand = Errand.None;
}
